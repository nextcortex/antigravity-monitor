...
(function () {
  ...
  let _aaEditorOpen = false;
  // Local draft state for the editor
  let _aaDraft = null;
  ...
  function handleAction(action, dataset) {
    switch (action) {
      case 'toggleAutoAccept':
        ...
        break;
      case 'toggleAutoAcceptEditor':
        _aaEditorOpen = !_aaEditorOpen;
        if (_aaEditorOpen && state && state.autoAcceptConfig) {
          _aaDraft = JSON.parse(JSON.stringify(state.autoAcceptConfig));
        }
        render(state);
        break;
      case 'openAutoAcceptSource':
        post('openAutoAcceptSource');
        break;
      case 'openAutoAcceptResearch':
        post('openAutoAcceptResearch');
        break;
  ...
  function renderAutoAcceptEditor(s) {
    ...
    html += `<div class="agm-section-header" data-action="toggleAutoAcceptEditor">
      <span class="agm-section-arrow ${_aaEditorOpen ? 'expanded' : ''}">▶</span>
    ...
    if (_aaEditorOpen) {
      ...
      html += `<button class="agm-action-btn" data-action="openAutoAcceptSource">📝 Edit Source</button>`;
      html += `<button class="agm-action-btn" data-action="openAutoAcceptResearch">📖 Research</button>`;
      html += `</div>`;

      html += `</div>`; // .agm-aa-editor
    }

    html += `</div>`; // .agm-section
    return html;
  }

  // Handle add-item buttons (these need special handling since they read input values)
  document.addEventListener('click', (e) => {
    const el = e.target;
    if (!el || !el.id) return;
    if (!_aaDraft) return;
